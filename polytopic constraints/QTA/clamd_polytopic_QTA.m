...
% Taylor Approximation" (QTA) in a poltopic constraint set. It
...
for mn = [0:4]
        ...
        for k = 1:NF
            ...
            for i = 1:length(V1(1,:))
                for j = 1:length(H(1,1,:))
                    conc(i,j) = V1(:,i)' * H(:,:,j) * V1(:,i)  +  c(:,:,j)' * V1(:,i)  +  0.25 * c(:,:,j)' * svdH(j).U1 / svdH(j).S1 * svdH(j).U1' * c(:,:,j) < 0.5;
                end
            end
            ...
            for i = 1:length(V1(1,:))
                ...
                for j2 = 1:nf
                    for j = j2+1:nf+1
                        obj(k,i) = obj(k,i) + V1(:,i)' * TH(:,:,count) * V1(:,i)  +  Tc(:,:,count)' * V1(:,i)  + Th(count);  % Taylor approximation
                        count = count+1;
                    end
                end
            end
    ...
    acc = 1 - sum(f)/kk;
    Nkm = mean(Nk);
    save(['clamd_polytopic_QTA_' num2str(mn)])
end
warning('on')