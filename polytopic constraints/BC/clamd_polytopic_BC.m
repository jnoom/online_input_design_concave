...
% Coefficient" (BC) in a polytopic constraint set. It produces a .mat-file
...
for mn = [0:4]
    ...
    for kk=1:Nexp
        ...
        for k = 1:NF
            ...
            for i = 1:length(V1(1,:))
                ...
                for j2 = 1:nf
                    for j = j2+1:nf+1
                        obj(k,i) = obj(k,i) + sqrt(P(k+1,j2,kk)*P(k+1,j,kk)) * exp(-V1(:,i)' * H(:,:,count) * V1(:,i)  -  c(:,:,count)' * V1(:,i)  -  h(count));  % Bhat. coefficient
                        count = count+1;
                    end
                end
            end
    ...
    acc = 1 - sum(f)/kk;
    Nkm = mean(Nk);
    save(['clamd_polytopic_BC_' num2str(mn)])
end
warning('on')
