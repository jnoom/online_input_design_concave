...
% Distance" (BD) in a polytopic constraint set. It produces a .mat-file
...
for mn = [0:4]  % Actual model (0 if fault-free,1:4 if fault)
    ...
    for kk=1:Nexp
        ...
        A = repmat(sys.A,1,1,nf+1); %repmat([0.95 0; 0 0.96],1,1,nf+1); %repmat([0.6 0.2; -0.2 0.7],1,1,nf+1);
        B = repmat([sys.B [0.5;0]],1,1,nf+1); %repmat([-0.3861 0.1994; -0.1994 0.3861],1,1,nf+1);
        ...
        for k = 1:NF
            ...
            for i = 1:length(V1(1,:))
                ...
                for j2 = 1:nf
                    for j = j2+1:nf+1
                        obj(k,i) = obj(k,i) - V1(:,i)' * H(:,:,count) * V1(:,i)  -  c(:,:,count)' * V1(:,i);  % Bhat. distance 
                        count = count+1;
                    end
                end
            end
    ...
    acc = 1 - sum(f)/kk;
    Nkm = mean(Nk);
    save(['clafd_polytopic_BD_' num2str(N) '_' num2str(mn)])
end
