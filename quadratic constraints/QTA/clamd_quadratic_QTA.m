...
for mn = [0:4]
    ...
    for kk=1:Nexp
        ...
        Ni = 1;   % Number of random initializations
        ...
        for k = 1:NF
            ...
            u_opt = double(py.dccp_BD_QTA.minsumTaylor(py.numpy.array(permute(TH,[3,2,1])),py.numpy.array(reshape(permute(Tc,[3,2,1]),[count-1,N*nu])),py.numpy.array(Th),uE,nu,Ni,str2double([num2str(seednumber) num2str(k)])));
            u(:,k) = u_opt(1:nu);
            for i = 1:nf+1
    ...
    acc = 1 - sum(f)/kk;
    Nkm = mean(Nk);
    save(['clamd_quadratic_QTA_' num2str(mn)])
end
