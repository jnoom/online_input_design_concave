...
% Bhattacharyya Coefficient" (SBC) in a quadratic constraint set. It
% requires the Python script "dccp_SBC.py" in the Current Folder and
...
for mn = [0:4]  % Actual model (0 if fault-free,1:4 if fault)
    ...
    for kk=1:Nexp
        ...
        for k = 1:NF
            ...
            for i = 1:nf
                for j = i+1:nf+1
                    ...
                    for j2 = 2:N
                        ...
                        c(:,:,count,j3)    = 0.5 * Gamma(1:j2*ny,:,count)' / Omega(1:j2*ny,1:j2*ny,count)  * Gamma2(1:j2*ny,:,count);
                        h(count,j3)        = 0.25 * Gamma2(1:j2*ny,:,count)' / Omega(1:j2*ny,1:j2*ny,count) * Gamma2(1:j2*ny,:,count)...
            ...
            u_opt = double(py.dccp_SBC.minsumbhat(py.numpy.array(permute(H,[4,3,2,1])),py.numpy.array(reshape(permute(c,[4,3,2,1]),[(N-1),count-1,(N-1)*nu])),py.numpy.array(permute(h,[2,1])),py.numpy.array(P(k+1,:,kk)),uE,nu,ccpN,str2double([num2str(seednumber) num2str(k)])));
            u(:,k+1) = u_opt(1:nu);
        end
        t1(kk)=toc;
        conc(kk) = min(concave(kk,1:k-1,:),[],'all');
...
%         legend('$i=0$','$i=1$','$i=2$','$i=3$','$i=4$','Interpreter','latex');
    ...
    acc = 1 - sum(f)/kk;
    Nkm = mean(Nk);
    save(['clafd_quadratic_SBC_' num2str(Npexp) '_' num2str(N) '_' num2str(mn)])
end
