% This script generates results for the closed-loop approach "Quadratic 
% Taylor Approximation" (QTA) in a quadratic constraint set. It 
% requires the Python script "dccp_BD_QTA.py" in the Current Folder and
...
for mn = [0:4]  % Actual model (0 if fault-free,1:4 if fault)
    ...
    for kk=1:Nexp
        ...
        ccpN = 1;       % Number of random initializations
        ...
        for k = 1:NF
            ...
            for i = 1:nf
                for j = i+1:nf+1
                    ...
                    fx0 = sqrt(P(k+1,i,kk) * P(k+1,j,kk)) * exp(- h(count));
                    TH(:,:,count) = 0.5 * fx0 * (c(:,:,count)*c(:,:,count)' - 2*H(:,:,count));
                    Tc(:,:,count) = - fx0 * c(:,:,count);
                    Th(count)     = fx0;
            ...
            u_opt = double(py.dccp_BD_QTA.minsumTaylor(py.numpy.array(permute(TH,[3,2,1])),py.numpy.array(reshape(permute(Tc,[3,2,1]),[count-1,(N-1)*nu])),py.numpy.array(Th),uE,nu,ccpN,str2double([num2str(seednumber) num2str(k)])));
            u(:,k+1) = u_opt(1:nu);
        end
        t1(kk) = toc;
...
%         legend('$i=0$','$i=1$','$i=2$','$i=3$','$i=4$','Interpreter','latex');
    ...
    acc = 1 - sum(f)/kk;
    Nkm = mean(Nk);
    save(['clafd_quadratic_QTA_' num2str(Npexp) '_' num2str(N) '_' num2str(mn)])
end
